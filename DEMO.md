# circuit-tracer dashboard — demo runbook

Objectif: présenter une démo concise d’explicabilité **mécanistique** basée sur les papiers *Transformer Circuits* (attribution graphs + interventions + validation).

Cette démo est organisée autour de:
- un **token cible** (logit cible, next‑token),
...
3) **Choisir le token cible**
- Colonne gauche → `Target logit`
...
4bis) **Inspecter rapidement une feature**
- Colonne droite → `Feature inspection (direct logit effects)`
...
6) **Exporter**
- `Download demo report (Markdown)` pour générer un rapport réutilisable
- `Download … JSON` si vous voulez réutiliser les résultats ailleurs
...
### 1bis) Swap “state → capital” (Dallas vs Oakland)
...
- Le but n’est pas de prétendre décrire “toute” la cognition du modèle, mais de tester une hypothèse par intervention.
- Il vaut mieux éviter les claims d’“introspection” au sens fort sans protocole dédié.
- Les artefacts exportés (`report.md` + JSON) servent surtout à garder une trace reproductible de la démo.
